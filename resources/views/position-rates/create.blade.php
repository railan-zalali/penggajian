<x-app-layout>
    <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-gray-800">{{ __('Tambah Tarif Gaji Jabatan') }}</h1>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <form method="POST" action="{{ route('settings.position-rates.store') }}" class="p-6">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">Contoh: Kades, Sekdes, Bendahara, dll.</p>
                    </div>
                    ...
                    <div>
                        <label for="working_days"
                            class="block text-sm font-medium text-gray-700 mb-1">{{ __('Jumlah Hari Kerja') }} <span
                                class="text-red-500">*</span></label>
                        <input type="number" id="working_days" name="working_days" value="{{ old('working_days', $workingDays) }}"
                            min="1" max="31" step="1"
                        ...
                        @error('working_days')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">Jumlah hari kerja dalam sebulan untuk menghitung tarif harian.
                        </p>
                    </div>
